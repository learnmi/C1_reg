...
for lp in lps:
    # Reg model
    ridge = Ridge(alpha=lp, max_iter=10000)
    ridge.fit(X,y)
    #predict
...
plt.ylabel("E-RMS")
plt.legend()
plt.figtext(.6, .6, "Best Alpha  = %s" % (str(test_score[test_score.index(min(test_score))][0])))
plt.figtext(.6, .5, "Best RMSE = %s" % (str(test_score[test_score.index(min(test_score))][1])))
plt.show()
